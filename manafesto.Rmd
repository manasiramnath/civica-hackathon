...
norway_plot <- ggplot(n, aes(x = partyname, y = rile, fill = factor(year))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Party", y = "Right-Left Score", title = "Right-Left Score in Norwegian Parties in 2015 and 2019", subtitle = "Labour Party had highest % of votes both years") +
  scale_fill_manual(values = c("2013" = "lightblue", "2017" = "purple"), name="Year") +  # Customizing fill colors
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Save Sweden plot as JPEG
ggsave("sweden_plot.jpeg", plot = sweden_plot, width = 8, height = 6, dpi = 300)
ggsave("denmark_plot.jpeg", plot = denmark_plot, width = 8, height = 6, dpi = 300)
ggsave("norway_plot.jpeg", plot = norway_plot, width = 8, height = 6, dpi = 300)
